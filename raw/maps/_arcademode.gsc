...
arcademode_dvar_init()
{
	...
	//player
	setdvar ( "arcademode_score_suicide",		-50 );
	setdvar ( "arcademode_score_laststand",		-10 );
	setdvar ( "arcademode_score_revive",			10 );
	
	setdvar ("arcademode_minimumAllowedSuicides", 5 );
		
	setdvar ("arcademode_minimumAllowedPoints", 	-1500 );
	setdvar ("arcademode_minimumAllowedWarning", -500 );
	...
	//Friendies AI
	setdvar ( "arcademode_friendies_damage",		-10 );	
	...
	setdvar ( "arcademode_score_explosion",		0 );
		
	level.arcadeMode_killBase = getDvarInt( "arcademode_score_kill" );
	
	//objects
	setdvar ( "arcademode_score_explodableitem",	1 );
	setdvar ( "arcademode_score_enemyexitingcar",10 );
	setdvar ( "arcademode_score_banzai",			15 );
	setdvar ( "arcademode_score_vehicle",		3 );
	setdvar ( "arcademode_score_dog",			5 );
	setdvar ( "arcademode_score_tankassist",		5 );	
	setdvar ( "arcademode_score_tank",			25 );
	setdvar ( "arcademode_score_tank_friendly",	-5 );
	setdvar ( "arcademode_score_tankmantle",		50 );
	setdvar ( "arcademode_score_watertower",		15 );
	setdvar ( "arcademode_score_treehugger",		10 );
	
	//actions
	setdvar ( "arcademode_score_bombplant",		20 );
	
	// SRS 8/12/2008: generic, for scripters to assign level-specific points
	setdvar ( "arcademode_score_generic100", 	10 );
	setdvar ( "arcademode_score_generic250", 	25 );
	setdvar ( "arcademode_score_generic500", 	50 );
	setdvar ( "arcademode_score_generic750", 	75 );
	setdvar ( "arcademode_score_generic1000", 	100 );

	//collectables
	setdvar ( "arcademode_score_berserker",		2 );
	setdvar ( "arcademode_score_hardcore",		1 );
	setdvar ( "arcademode_score_body_armor",		2 );
	setdvar ( "arcademode_score_thunder",		1 );
	setdvar ( "arcademode_score_zombie",			1 );
	setdvar ( "arcademode_score_vampire",		1 );
	setdvar ( "arcademode_score_dirtyharry",		1 );
	setdvar ( "arcademode_score_hard_headed",	2 );
	setdvar ( "arcademode_score_dead_hands",		2 );
	setdvar ( "arcademode_score_sticksstones",	2 );
	setdvar ( "arcademode_score_flak_jacket",	1 );
	setdvar ( "arcademode_score_morphine",		1 );	
	
	//Round Bonus	
	setdvar ( "arcademode_bonus_hardest2kill",	100 );
	setdvar ( "arcademode_bonus_mostrevives",	50 );
	setdvar ( "arcademode_bonus_mostheadshots",	50 );	
		
	// KILL STREAKS	
	setdvar ( "arcademode_ks_2",	2 );
	setdvar ( "arcademode_ks_3",	3 );
	setdvar ( "arcademode_ks_4",	4 );
	setdvar ( "arcademode_ks_5",	5 );
	setdvar ( "arcademode_ks_6",	6 );
	setdvar ( "arcademode_ks_7",	7 );
	setdvar ( "arcademode_ks_8",	8 );
	setdvar ( "arcademode_ks_9",	9 );
	setdvar ( "arcademode_ks_10",   10 );
	
	setdvar ( "arcademode_ks_time_2", 7 );
	setdvar ( "arcademode_ks_time_3", 5 );
	setdvar ( "arcademode_ks_time_4", 5 );
	setdvar ( "arcademode_ks_time_5", 5 );
	setdvar ( "arcademode_ks_time_6", 4 );
	setdvar ( "arcademode_ks_time_7", 4 );
	setdvar ( "arcademode_ks_time_8", 3 );	
	setdvar ( "arcademode_ks_time_9", 10  );
	setdvar ( "arcademode_ks_time_10", 10  );	
	
	setdvar ( "arcademode_ks_maxtime", 15 );	
	...
	skill_multiplier = [];
	skill_multiplier[ 0 ] = 0.5;
	skill_multiplier[ 1 ] = 1;
	skill_multiplier[ 2 ] = 2;
	skill_multiplier[ 3 ] = 3;
...
challengeWatcher(hit_location) {
	...
	maxKillChallenge = int(tablelookup("mp/weaponchallengetable.csv",1,weapName,8));
	maxHsChallenge = int(tablelookup("mp/weaponchallengetable.csv",1,weapName,9));
...
/*
	...
	*/
	...
	if(!isdefined(maxKillChallenge))
		return;

	//Kills challenges
	switch(currentTierStatCheckKill) {
		case 1:
			self iPrintLn("TIER 1");
			targetKill = int(tablelookup("mp/weaponchallengetable.csv",1,weapName,10));
			...
			if(currentKillStats < targetKill) {
				...
				if(currentKillStats >= targetKill) {
					...
					challengesSummary += 100;
				}
			}
			break;
		case 2:
			self iPrintLn("TIER 2");
			targetKill = int(tablelookup("mp/weaponchallengetable.csv",1,weapName,11));
			currentKillStats = self getStat(int(tablelookup("mp/weaponchallengetable.csv",1,weapName,4)));
			...
			if(currentKillStats < targetKill) {
				...
				if(currentKillStats >= targetKill) {
					...
					challengesSummary += 500;
				}
			}
			break;
		case 3:
			self iPrintLn("TIER 3");
			targetKill = int(tablelookup("mp/weaponchallengetable.csv",1,weapName,12));
			currentKillStats = self getStat(int(tablelookup("mp/weaponchallengetable.csv",1,weapName,4)));
			...
			if(currentKillStats < targetKill) {
				if(currentKillStats >= maxKillChallenge)
					currentKillStats = 255;
				else
					currentKillStats += 1;
				...
				if(currentKillStats >= targetKill) {
					...
					challengesSummary += 1000;
				}
			}
			break;
		case 4:
			self iPrintLn("TIER 4");
			targetKill = int(tablelookup("mp/weaponchallengetable.csv",1,weapName,13));
			currentKillStats = self getStat(int(tablelookup("mp/weaponchallengetable.csv",1,weapName,4)));

			if(targetKill == 0) {
				self setStat(int(tablelookup("mp/weaponchallengetable.csv",1,weapName,2)), 255);
				break;
			}
			...
			if(currentKillStats < targetKill) {
				...
				if(currentKillStats >= targetKill) {
					...
					challengesSummary += 1500;
				}
			}
			break;
		default:
			self iPrintLn("NO TIER");
			break;
	}
	...
	switch(currentTierStatCheckHS) {
		case 1:
			self iPrintLn("TIER 1");
			targetKill = int(tablelookup("mp/weaponchallengetable.csv",1,weapName,14));
			currentKillStats = self getStat(int(tablelookup("mp/weaponchallengetable.csv",1,weapName,5)));
			...
			if(currentKillStats < targetKill) {
				currentKillStats += 1;
				self setStat(int(tablelookup("mp/weaponchallengetable.csv",1,weapName,5)), currentKillStats);
				if(currentKillStats >= targetKill) {
					currentTierStatCheckHS += 1;
					self thread maps\_challenges_coop::challengeNotify(&"CHALLENGE_"+tablelookup("mp/challengetable_tier2.csv",0,4,8));
		...
		case 2:
			self iPrintLn("TIER 2");
			targetKill = int(tablelookup("mp/weaponchallengetable.csv",1,weapName,15));
			currentKillStats = self getStat(int(tablelookup("mp/weaponchallengetable.csv",1,weapName,5)));
			...
			if(currentKillStats < targetKill) {
				...
				if(currentKillStats >= targetKill) {
					currentTierStatCheckHS += 1;
					self thread maps\_challenges_coop::challengeNotify(&"CHALLENGE_"+tablelookup("mp/challengetable_tier2.csv",0,5,8));
					self setStat(int(tablelookup("mp/weaponchallengetable.csv",1,weapName,3)), currentTierStatCheckHS);
					self thread updatePlusScoreHUD( 500 );
					challengesSummary += 500;
				}
		...
		case 3:
			self iPrintLn("TIER 3");
			targetKill = int(tablelookup("mp/weaponchallengetable.csv",1,weapName,16));
			currentKillStats = self getStat(int(tablelookup("mp/weaponchallengetable.csv",1,weapName,5)));
			...
			if(currentKillStats < targetKill) {
				...
				if(currentKillStats >= targetKill) {
					...
					self thread maps\_challenges_coop::challengeNotify(&"CHALLENGE_"+tablelookup("mp/challengetable_tier2.csv",0,6,8));
					self setStat(int(tablelookup("mp/weaponchallengetable.csv",1,weapName,3)), currentTierStatCheckHS);
					self thread updatePlusScoreHUD( 1000 );
					challengesSummary += 1000;
				}
	...
	if(currentTierStatCheckKill != 0 && currentTierStatCheckKill < 255) {
		...
		if(currentKillStats < int(tablelookup("mp/weaponchallengetable.csv",1,weapName,8))) {
			...
			} else if(currentKillStats >= maxKillChallenge && currentTierStatCheckKill == 3) {
				currentTierStatCheckKill = 255;
				self thread maps\_challenges_coop::challengeNotify(&"CHALLENGE_"+tablelookup("mp/challengetable_tier2.csv",0,3,8));
				self setStat(int(tablelookup("mp/weaponchallengetable.csv",1,weapName,2)), currentTierStatCheckKill);
				self setStat(int(tablelookup("mp/weaponchallengetable.csv",1,weapName,6)), 983055);
...
updatePlusScoreHUD( amount )
{
	...
	self maps\_challenges_coop::giveRankXP("kill", amount);
