...
init()
{
	...
	precacheShader( "hud_xpticker480" );
	precacheShader( "hud_xpticker480ws" );
	*/
...
{
	level.barElem = newHudElem();
	level.barElem.alignX = "left";
	level.barElem.alignY = "top";
	level.barElem.horzAlign = "center";
	level.barElem.vertAlign = "top";
	level.barElem.x = -125;
	level.barElem.y = 10;
	level.barElem.frac = 0;
	level.barElem.color = color;
	level.barElem.sort = -2;
	level.barElem.alpha = 0;
	level.barElem.shader = "scorebar_fadein";
	level.barElem setShader( "scorebar_fadein", width, height - 2 );
	level.barElem.hidden = false;
	if ( isDefined( flashFrac ) )
	{
		level.barElem.flashFrac = flashFrac;
	}
	...
	barElemFrame.bar = level.barElem;
	...
	barElemBG.vertAlign = "top";
	barElemBG.alignX = "center";
	barElemBG.alignY = "top";
	barElemBG.width = width;
	barElemBG.height = height;
	barElemBG.xOffset = 0;
	barElemBG.yOffset = 0;
	barElemBG.bar = level.barElem;
	barElemBG.barFrame = barElemFrame;
	barElemBG.children = [];
	barElemBG.sort = -3;
	barElemBG.color = (1,1,1);
	barElemBG.alpha = 0.5;
	barElemBG.shader = "hud_xpticker480";
	barElemBG setShader( "hud_xpticker480", 2 + 0, 5 + 0 );
	barElemBG.hidden = false;
	
	level.rank_hud_icon_c = newHudElem( );
	level.rank_hud_icon_c.foreground = true; 
	level.rank_hud_icon_c.sort = 2; 
	level.rank_hud_icon_c.hidewheninmenu = false; 
	level.rank_hud_icon_c.alignX = "center"; 
	level.rank_hud_icon_c.alignY = "top";
	level.rank_hud_icon_c.horzAlign = "center"; 
	level.rank_hud_icon_c.vertAlign = "top";
	level.rank_hud_icon_c.x = -150; 
	level.rank_hud_icon_c.y = 9; 
	level.rank_hud_icon_c.alpha = 1;
	
	level.rank_hud_icon_n = newHudElem( );
	level.rank_hud_icon_n.foreground = true; 
	level.rank_hud_icon_n.sort = 2; 
	level.rank_hud_icon_n.hidewheninmenu = false; 
	level.rank_hud_icon_n.alignX = "center"; 
	level.rank_hud_icon_n.alignY = "top";
	level.rank_hud_icon_n.horzAlign = "center"; 
	level.rank_hud_icon_n.vertAlign = "top";
	level.rank_hud_icon_n.x = 150; 
	level.rank_hud_icon_n.y = 9; 
	level.rank_hud_icon_n.alpha = 1;

	//return barElemBG;
}

barIncXp(p_rankxp, n_rankxpmin, n_rankxpmax)
{
	player = get_players()[0];
	p_rank = player getRank();
	n_rankicon = player getRankInfoIcon(p_rank + 1, self.prestige);
	p_rankicon = player getRankInfoIcon(p_rank, self.prestige);
	
	p_bar = (-1*(int(((n_rankxpmin - p_rankxp) / (n_rankxpmax))*250)));
	level.barElem ScaleOverTime( 0.1, p_bar, 15-2);
	
	level.rank_hud_icon_c SetShader(p_rankicon, 25, 25);
	level.rank_hud_icon_n SetShader(n_rankicon, 25, 25);
}
*/
...
onPlayerConnect()
{
	for(;;)
	{
		...
		//barIncXp(player getRankXP(), player getRankInfoMinXP(self.rank), player getRankInfoXPAmt(self.rank));
...
giveRankXP( type, value, levelEnd )
{
	...
	value = int( value * 1/*level.xpScale*/ );
...
updateRank()
{
	...
	//barIncXp(getRankXP(), getRankInfoMinXP(self.rank), getRankInfoXPAmt(self.rank));
...
{
	if ( isDefined(level) && isDefined(level.rankTable) ) { return int(level.rankTable[rankId][7]); }
	else return 0;
}
...
ch_assists( player )
{
	...
	difficulty = getdvar("currentDifficulty");
	
	switch(difficulty)
	{	
		case "easy":
			player maps\_challenges_coop::giveRankXP( "assist", 1 );
			//arcademode_assignpoints("arcademode_score_assist", player);
			//add_fake_xp(1);
			break;
		case "normal":
			player maps\_challenges_coop::giveRankXP( "assist", 2 );
			//arcademode_assignpoints("arcademode_score_assist", player);
			//add_fake_xp(2);
			break;
		case "hardened":
			player maps\_challenges_coop::giveRankXP( "assist", 5 );
			//arcademode_assignpoints("arcademode_score_assist", player);
			//add_fake_xp(5);
			break;
		case "veteran":
			player maps\_challenges_coop::giveRankXP( "assist", 8 );
			//arcademode_assignpoints("arcademode_score_assist", player);
			//add_fake_xp(8);
			break;
	}
	
	player maps\_challenges_coop::statAdd("assists", 1);
}
